...
export default function Passanger1(props) {
  // Modal
  ...
  const [children, setchildren] = useState(props.childrenData);
  const [adult, setadult] = useState(props.adultData);
  const [Infant, setInfant] = useState(props.infantData);
  ...
  function show() {
    ...
    document
      .querySelectorAll(
        ".menuflitem-2,.menuflitem-3,.menuflitem-3.menuflitem-4,.menuflitem-4,.menuflitem-5,.menuflitem-6"
      )
      .forEach(function (el) {
    ...
    document
      .querySelectorAll(
        ".menuflitem-2,.menuflitem-3,.menuflitemb-3.menuflitem-4,.menuflitemb-4"
      )
      .forEach(function (el) {
        el.style.display = "none";
      });
  ...
  return (
    <>
      <Listbox value={selected} onChange={setSelected}>
        ...
        >
          <Listbox.Button
            className="btnid-4 text-left button-infant-parent button-infant d-flex  align-items-center justify-content-center w-100"
            onClick={() => show()}
          >
